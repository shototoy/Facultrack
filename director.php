...
// Enhanced AJAX endpoint for real-time dashboard updates (both director and program)
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'get_dashboard_data') {
    // Allow both campus_director and program_chair
    if (!in_array($_SESSION['role'], ['campus_director', 'program_chair'])) {
    ...
    try {
        ...
        if ($user_role === 'campus_director') {
        ...
        } else if ($user_role === 'program_chair') {
            // Program Chair: Return all data (for card-based interface)
            $user_id = $_SESSION['user_id'];
            
            // Get program chair's classes
            ...
            // Get all faculty (for faculty cards)
            $response_data['faculty_data'] = getAllFaculty($pdo);
            
            // Get all courses (for course cards)
            $response_data['courses_data'] = getAllCourses($pdo);
        }
    ...
    try {
        ...
        sendJsonResponse([
            ...
            'statistics' => [
                ...
                'active_announcements' => count($announcements_data),
                'available_faculty' => $online_faculty
            ],
            'timestamp' => date('Y-m-d H:i:s')
        ]);
